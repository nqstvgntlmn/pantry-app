...
### My Recipes
- **Floating search FAB:** A compact magnifying glass button floats in the top-right corner of the screen (below the "Make now" button area). It is **hidden when the user is at the top** and only appears after scrolling down 60px+ in the recipe list — this prevents clutter when the header is visible. Tapping expands a search panel that slides **down from the top** of the screen (pinned below the filter chips) with search bar, sort dropdown (A→Z / Newest / Highest rated), and all filter options (difficulty, cook time, serves, protein, tags). Tapping outside the panel or the backdrop collapses it. The FAB is a fixed-position body element (not inline in `.rtabs`) so it doesn't squish filter chips. Hidden on non-recipe tabs via `hideRecSearchFab()`. Highlights in accent color when filters are active.
...
- **Session 15:** Two UX features:
  1. **Floating Recipe Search FAB:** A compact magnifying glass button (`.rec-search-fab`) floats fixed in the top-right corner of the screen, below the "Make now" button area. **Hidden by default** — only appears after the user scrolls down 60px+ in the recipe list (scroll listener on `#rbody`). Tapping opens a frosted-glass search panel (`rec-search-panel`) that slides **down from the top** (pinned below filter chips) with search bar, sort dropdown, and full filter options. Tapping outside (backdrop) collapses it. FAB highlights in accent color when filters are active. Panel auto-focuses search input. FAB is a body-level fixed element (not inline in `.rtabs`) — prevents squishing filter chips. Hidden on non-recipe tabs via `hideRecSearchFab()`. Auto-closes when user scrolls back to top. Exports: `toggleRecSearchPanel`, `openRecSearchPanel`, `closeRecSearchPanel`, `hideRecSearchFab`. **History:** Originally a floating bottom-right FAB (Session 15), moved to inline chip row (Session 17), moved back to floating top-right FAB with scroll-based visibility (Session 18) to fix Community chip squishing and position the panel at the top.
  2. **Status Bar Tap-to-Scroll-Top:** Global feature across all tabs — invisible `.status-bar-tap` div at the very top of the viewport. Height uses `calc(env(safe-area-inset-top, 20px) + 15px)` to cover the absolute topmost edge of the screen including the clock/Dynamic Island area on all iPhone models. z-index 9999 ensures nothing can sit above the tap zone. Tapping it smoothly scrolls the active tab's content to the top, mimicking native iOS behavior that PWAs don't get automatically. Skips scroll when overlays/modals are open. Uses `_currentTab()` to find the active screen's scrollable child container. **History:** Originally used `env(safe-area-inset-top)` + 44px min-height (Session 15), reduced to strict 20px (Session 17), expanded to `env(safe-area-inset-top) + 15px` (Session 18) to reach the very top edge.
