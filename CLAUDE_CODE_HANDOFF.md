...
- **Session 15:** Two UX features:
  1. **Floating Recipe Search FAB:** A compact magnifying glass button (`.rec-search-fab`) floats fixed in the top-right corner of the screen, **below the entire non-scrolling header** (title row + filter chips). Positioned at `top: calc(env(safe-area-inset-top) + 120px)` so it floats over the scrollable recipe content (rbody) and never overlaps any filter chip including "Community". **Hidden by default** — only appears after the user scrolls down 60px+ in the recipe list (scroll listener on `#rbody`). Tapping opens a frosted-glass search panel (`rec-search-panel`) that slides **down from the top** (pinned at +130px) with search bar, sort dropdown, and full filter options. Tapping outside (backdrop) collapses it. FAB highlights in accent color when filters are active. Panel auto-focuses search input. FAB is a body-level fixed element (not inline in `.rtabs`) — prevents squishing filter chips. Hidden on non-recipe tabs via `hideRecSearchFab()`. Auto-closes when user scrolls back to top. Exports: `toggleRecSearchPanel`, `openRecSearchPanel`, `closeRecSearchPanel`, `hideRecSearchFab`. **History:** Originally a floating bottom-right FAB (Session 15), moved to inline chip row (Session 17), moved back to floating top-right FAB with scroll-based visibility (Session 18) to fix Community chip squishing. Moved further down to +120px (Session 19) to clear all filter chips.
  2. **Tap-to-Scroll-Top:** Completely rewritten. A transparent fixed `.scroll-top-tap` div spans the full screen width at `top:0`, `height:60px`, `z-index:9999`. This is a large, simple tap target that reliably receives touches across all iPhone models. Tapping it smooth-scrolls the active tab's content to the top. Uses a `scrollSelectors` map to find each tab's scrollable container by known CSS selector (`.hbody`, `.ibody`, `.rbody`, `#sh-list-body`, `.chmsgs`). Skips scroll when overlays/modals are open. **History:** Originally used `env(safe-area-inset-top)` + 44px (Session 15), reduced to 20px (Session 17), expanded to `env(safe-area-inset-top) + 15px` (Session 18), fully rewritten to simple 60px fixed div (Session 19) after persistent tap-detection issues.
