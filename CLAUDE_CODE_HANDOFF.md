...
### My Recipes
- **Floating search FAB:** A compact magnifying glass button at fixed pixel coordinates (`top:120px; right:16px`) — not relative to any other element. This ensures it never touches the filter chips row or the "Make now" button. It is **hidden when the user is at the top** and only appears after scrolling down 60px+ in the recipe list. Tapping expands a search panel that slides **down from the top** (at `top:130px`, just below the FAB) with search bar, sort dropdown (A→Z / Newest / Highest rated), and all filter options (difficulty, cook time, serves, protein, tags). Tapping outside the panel or the backdrop collapses it. The FAB is a fixed-position body element (not inline in `.rtabs`) so it doesn't squish filter chips. Hidden on non-recipe tabs via `hideRecSearchFab()`. Highlights in accent color when filters are active.
...
- **Session 15:** Two UX features:
  1. **Floating Recipe Search FAB:** A compact magnifying glass button (`.rec-search-fab`) at fixed pixel coordinates `top:120px; right:16px` — not positioned relative to any other element, so it never touches the filter chips row or the "Make now" button. **Hidden by default** — only appears after the user scrolls down 60px+ in the recipe list (scroll listener on `#rbody`). Tapping opens a frosted-glass search panel (`rec-search-panel`) at `top:130px` (just below the FAB) with search bar, sort dropdown, and full filter options. Tapping outside (backdrop) collapses it. FAB highlights in accent color when filters are active. Panel auto-focuses search input. FAB is a body-level fixed element (not inline in `.rtabs`) — prevents squishing filter chips. Hidden on non-recipe tabs via `hideRecSearchFab()`. Auto-closes when user scrolls back to top. Exports: `toggleRecSearchPanel`, `openRecSearchPanel`, `closeRecSearchPanel`, `hideRecSearchFab`. **History:** Originally a floating bottom-right FAB (Session 15), moved to inline chip row (Session 17), moved back to floating top-right FAB with scroll-based visibility (Session 18) to fix Community chip squishing. Moved further down to +120px (Session 19) to clear all filter chips. Changed to fixed pixel coordinates (Session 20) to eliminate safe-area-inset dependency.
