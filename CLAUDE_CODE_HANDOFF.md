...
### My Recipes
- **Inline search button:** A compact magnifying glass button sits at the end of the `.rtabs` filter chip row (right of "Community"). It's always visible on the Recipes tab — no scroll threshold needed. Tapping expands into a full overlay panel with search bar, sort dropdown (A→Z / Newest / Highest rated), and all filter options (difficulty, cook time, serves, protein, tags). Tapping outside the panel or the backdrop collapses it. Since the button is inside the `#screen-recipes` DOM, it automatically hides on other tabs. Highlights in accent color when filters are active.
...
- Invisible touch target in the top 20px of the screen only (`z-index: 9999`). Hard-coded to 20px instead of `env(safe-area-inset-top)` to avoid blocking header buttons on notch/Dynamic Island devices where the safe area can be 47-59px.
...
- **Session 15:** Two UX features:
  1. **Inline Recipe Search Button:** A compact magnifying glass button (`rec-search-inline-btn`) sits inline at the end of the `.rtabs` filter chip row, right of the "Community" chip. Always visible on the Recipes tab — no scroll threshold needed. Tapping opens a frosted-glass overlay panel (`rec-search-panel`) with search bar, sort dropdown, and full filter options. Tapping outside (backdrop) collapses it. Button highlights in accent color when filters are active. Panel auto-focuses search input. Since the button is inside `#screen-recipes`, it automatically hides on other tabs — no manual visibility management needed. Exports: `toggleRecSearchPanel`, `openRecSearchPanel`, `closeRecSearchPanel`, `hideRecSearchFab`. **History:** Originally a floating bottom-right FAB (Session 15), moved to inline chip row (Session 17) to avoid overlap with "Make now" button and prevent appearing on non-Recipes tabs.
  2. **Status Bar Tap-to-Scroll-Top:** Global feature across all tabs — invisible `.status-bar-tap` div at the very top of the viewport. Height is hard-coded to `20px` (not `env(safe-area-inset-top)` which can be 47-59px on notch/Dynamic Island devices and blocks header buttons). z-index 9999 ensures nothing can sit above the tap zone. Tapping it smoothly scrolls the active tab's content to the top, mimicking native iOS behavior that PWAs don't get automatically. Skips scroll when overlays/modals are open. Uses `_currentTab()` to find the active screen's scrollable child container. **History:** Originally used `env(safe-area-inset-top)` + 44px min-height (Session 15), reduced to strict 20px (Session 17) to stop blocking header buttons.
