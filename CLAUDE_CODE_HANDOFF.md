...
### My Recipes
- **Floating search FAB:** A compact magnifying glass button at fixed pixel coordinates (`top:128px; right:16px`) — 8px lower than previous to clear the filter chips row by 5-10px. **Hidden when within 100px** of the top (inline search/sort/filters are visible there). Appears after scrolling past 100px in the recipe list. Inline search fades out (opacity transition) when FAB appears, fades back in when scrolling to top. Tapping the FAB expands a search panel that slides down from `top:138px` (just below the FAB) with search bar, sort dropdown (A→Z / Newest / Highest rated), and all filter options (difficulty, cook time, serves, protein, tags). Tapping outside the panel or the backdrop collapses it. The FAB is a fixed-position body element (not inline in `.rtabs`) so it doesn't squish filter chips. Hidden on non-recipe tabs via `hideRecSearchFab()`. Highlights in accent color when filters are active.
