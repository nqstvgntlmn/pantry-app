...
async function searchSpoonacular(query) {
  ...
  try {
    ...
    return (d.products || []).slice(0, 5).map(p => {
      // [IMAGES DISABLED] — image extraction commented out
...
async function searchKroger(query) {
  ...
  try {
    ...
    return (d.data || []).slice(0, 5).map(p => {
      // [IMAGES DISABLED] — Kroger image extraction commented out
      // const images = p.images || [];
      // const frontImg = images.find(img => img.perspective === "front");
      // const anyImg = frontImg || images[0];
  ...
  try {
    ...
    return hints.slice(0, 5).map(h => {
      ...
      return {
        ...
        // [IMAGES DISABLED] image: isValidImageUrl(f.image) ? f.image : null,
        image: null,
        source: "Edamam",
  ...
  try {
    ...
    return products.slice(0, 5).map(p => {
      ...
      return {
        ...
        // [IMAGES DISABLED] — Open Food Facts image preference chain commented out
...
async function searchUpcItemDb(query) {
  try {
    ...
    return (d.items || []).slice(0, 5).map(i => ({
      ...
      // [IMAGES DISABLED] image: (i.images || []).find(img => isValidImageUrl(img)) || null,
      image: null,
      source: "UPC Item DB",
  ...
  try {
    ...
    return (d.products || []).slice(0, 5).map(p => {
      ...
      return {
        ...
        // [IMAGES DISABLED] image: p.image_front_url || null,
        image: null,
...
export default async function handler(req, res) {
  ...
  // [IMAGES DISABLED] — Pre-check image lookup commented out
  ...
  function addResults(results) {
    ...
    for (const r of results) {
      ...
      // [IMAGES DISABLED] — image validation and illustration downgrade commented out
      ...
      if (seen.has(key)) {
        // [IMAGES DISABLED] — duplicate image upgrade commented out
        // if (r.image && !isIngredientIllustration(r.image)) {
        ...
        //     existing._imageSource = r.source;
        //   }
        // }
        continue;
      }
      seen.add(key);
      // [IMAGES DISABLED] if (r.image) r._imageSource = r.source;
      allResults.push(r);
      added++;
    }
    return added;
  ...
  function applyFallbackImages(results) {
    // No-op while images are disabled
    // for (const r of results) {
    //   if (!r.image) {
    //     if (r._fallbackImg) {
  ...
  // [IMAGES DISABLED] — image-based short-circuit simplified to count-only check
  ...
  if (allResults.length >= ENOUGH) {
    ...
    // [IMAGES DISABLED] applyFallbackImages(slice1);
    // [IMAGES DISABLED] if (imageDismissedFlag) { slice1.forEach(r => { r.image = null; }); }
  ...
  // [IMAGES DISABLED] applyFallbackImages(final);
  // [IMAGES DISABLED] if (imageDismissedFlag) { final.forEach(r => { r.image = null; }); }
