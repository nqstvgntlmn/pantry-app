...
Recipe: ${e.description}`}]})})).json(),r=(s.content&&s.content[0]&&s.content[0].text||"").replace(/```json|```/g,"").trim(),c=JSON.parse(r).filter(u=>!t.some(d=>d.includes(u.toLowerCase())||u.toLowerCase().includes(d)));if(!c.length){P("All ingredients already in pantry ✓");return}for(const u of c)await Ve({id:"shop-"+Date.now()+"-"+Math.random().toString(36).slice(2),name:u,qty:1,checked:!1,src:"recipe"});P(`Added ${c.length} ingredient${c.length!==1?"s":""} to shopping list 🛒`),Ae("erec"),window.showScreen("shopping")}catch{P("Couldn't parse ingredients")}}function LE(n,e){p.nr=n,e==="r"?Mi("rstars",n):e==="c"?Mi("cstars",n):e==="e"&&Mi("estars",n)}async function DE(n){const e=p.recs.find(r=>r.id===n);if(!e)return;const t=!e.isPublic,i=De(),s=(i==null?void 0:i.displayName)||localStorage.getItem("ks-who")||"Anonymous";t?(await Vv(e,s,p.hid),P("Recipe shared with the community!")):(await $v(e.id),P("Recipe removed from community")),await Bt({...e,isPublic:t})}async function ac(){const n=f("rbody");if(n){n.innerHTML='<div class="es"><div class="ei">🌍</div><p>Loading community recipes…</p></div>';try{p.comRecs=await Fv(),cc()}catch(e){console.error("loadCommunity:",e),n.innerHTML=`<div class="es"><div class="ei">⚠️</div><p>Couldn't load community recipes.</p></div>`}}}function NE(n){p.comCuisine=n,cc()}function ME(n){p.comSearch=n,cc()}function cc(){const n=f("rbody");if(!n)return;let e=[...p.comRecs];if(p.comCuisine&&p.comCuisine!=="all"&&(e=e.filter(s=>(s.cuisine||"").toLowerCase().includes(p.comCuisine.toLowerCase())||(s.tags||[]).some(r=>r.toLowerCase().includes(p.comCuisine.toLowerCase())))),p.comSearch){const s=p.comSearch.toLowerCase();e=e.filter(r=>(r.title||"").toLowerCase().includes(s)||(r.tags||[]).join(" ").toLowerCase().includes(s)||(r.cuisine||"").toLowerCase().includes(s)||(r.authorName||"").toLowerCase().includes(s))}e.sort((s,r)=>new Date(r.createdAt||0)-new Date(s.createdAt||0));const t=f("rsub");t&&(t.textContent=e.length+" community recipe"+(e.length!==1?"s":""));let i=`<div style="margin-bottom:14px">
    ...
    </div>`,wt("erec")}async function VE(n){if(!De()){P("Sign in to like recipes");return}const t=p.myLikes.has(n);try{await Uv(n,t),t?p.myLikes.delete(n):p.myLikes.add(n);const i=p.comRecs.find(r=>r.id===n);i&&(i.likes=(i.likes||0)+(t?-1:1));const s=f("com-like-btn");if(s){const r=p.myLikes.has(n);s.className=`btn ${r?"bp":"bs"} bsm`,s.innerHTML=`${r?"❤️":"🤍"} ${(i==null?void 0:i.likes)||0} Like${((i==null?void 0:i.likes)||0)!==1?"s":""}`}P(t?"Like removed":"Liked!")}catch(i){console.error("likeComRecipe:",i),P("Couldn't update like")}}async function $E(n){if(!De()){P("Sign in to save recipes");return}const t=p.comRecs.find(i=>i.id===n);if(t)try{await qv(t),P("Recipe saved to your kitchen! 📖"),Ae("erec")}catch(i){console.error("saveComToKitchen:",i),P("Couldn't save recipe")}}async function FE(n){var r;const e=De();if(!e){P("Sign in to comment");return}const t=f("com-cmt-input"),i=(r=t==null?void 0:t.value)==null?void 0:r.trim();if(!i)return;const s=e.displayName||localStorage.getItem("ks-who")||"Anonymous";try{const a=await jv(n,i,s);t.value="";const c=f("com-comments");c&&a&&(c.querySelector("div[style*='color:var(--mt)']")&&!c.querySelector("div[style*='border-bottom']")&&(c.innerHTML=""),c.innerHTML+=`<div style="padding:10px 0;border-bottom:1px solid var(--b1)">
        <div style="display:flex;justify-content:space-between;align-items:center">
