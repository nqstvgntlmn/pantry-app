...
`);navigator.share?navigator.share({title:"Shopping List",text:n}).catch(()=>{}):navigator.clipboard&&navigator.clipboard.writeText(n).then(()=>f("List copied!"))}function nd(){const t=a.shop.filter(n=>n.checked);if(!t.length){f("No completed items!");return}const e=c("atk-body");e.innerHTML=`<div style="padding:16px">
  ...
  </div>`,ye("atk")}function id(t,e,n){const i=c("atk-"+t);i.dataset.loc=e,i.querySelectorAll(".atk-loc button").forEach(s=>s.classList.remove("sel")),n.classList.add("sel")}async function sd(){const t=a.shop.filter(i=>i.checked),e=new Date().toLocaleDateString();let n=0;for(const i of t){const s=c("atk-"+i.id);if(!s)continue;const r=s.dataset.loc||$s(i.name),o=a.inv.find(d=>d.name.toLowerCase()===i.name.toLowerCase()),l=i.qty||1;await oe({id:o?o.id:"inv-"+Date.now()+"-"+Math.random().toString(36).slice(2),name:o?o.name:i.name,qty:o?o.qty+l:l,unit:o?o.unit:"unit",location:r,category:o?o.category:Ze({name:i.name}),addedAt:o?o.addedAt:e,brand:o?o.brand:"",expiry:o?o.expiry:null,image:o?o.image:null,source:"shopping"}),await Ot(i.id),n++}L("atk"),f(`${n} item${n!==1?"s":""} added to your kitchen! 🧺`)}async function rd(){const t=He().map(s=>{const r=s.toISOString().split("T")[0];return a.mp[r]?`${s.toLocaleDateString("en-US",{weekday:"short"})}: ${a.mp[r]}`:""}).filter(Boolean).join(", ");if(!t){f("No meals planned yet!");return}const e=a.inv.map(s=>`${s.name} (${s.qty} ${s.unit})`).join(", "),n=document.querySelector('[onclick="buildList()"]'),i=n?n.textContent:"";n&&(n.disabled=!0,n.textContent="⏳ Thinking…");try{const r=await(await fetch("/api/proxy",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:500,messages:[{role:"user",content:`Week meals: ${t}
Already have: ${e}
...
Be concise. Use what they have. Suggest variety — lean toward Bangladeshi and Turkish — avoid repeating recent meals. Format grocery lists as bullet points starting "- ".`}function Md(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>").replace(/^\d+\.\s+(.+)$/gm,"<li>$1</li>").replace(/^[-•]\s+(.+)$/gm,"<li>$1</li>").replace(/\n/g,"<br>")}async function Bs(){const t=c("chi"),e=t.value.trim();if(!e)return;t.value="",zs(t),a.chat.push({role:"user",content:e}),Qt("user",e);const n=c("csb");n&&(n.disabled=!0);const i="thinking-"+Date.now(),s=c("chmsgs");s.innerHTML+=`<div class="cb asst thinking" id="${i}">Thinking…</div>`,s.scrollTop=s.scrollHeight;try{const o=await(await fetch("/api/proxy",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:800,system:Nd(),messages:a.chat.map(u=>({role:u.role,content:u.content}))})})).json(),l=o.content&&o.content[0]&&o.content[0].text||"Sorry, I couldn't process that.",d=c(i);d&&d.remove(),a.chat.push({role:"assistant",content:l}),Qt("assistant",l)}catch{const o=c(i);o&&o.remove(),Qt("assistant","Sorry, I couldn't reach Claude. Check your connection and try again.")}n&&(n.disabled=!1)}function Qt(t,e){const n=c("chmsgs");if(!n)return;const i=document.createElement("div");i.className="cb "+(t==="user"?"user":"asst"),i.innerHTML=t==="user"?e:Md(e),n.appendChild(i),n.scrollTop=n.scrollHeight}function Od(t){const e=c("chi");e&&(e.value=t.textContent),Bs()}function Ud(){a.chat=[];const t=c("chmsgs");t&&(t.innerHTML='<div class="cb asst">Hey! 👋 I know your full inventory, meal plan, and saved recipes. What do you need?</div>')}function zs(t){t.style.height="auto",t.style.height=Math.min(t.scrollHeight,120)+"px"}const Hd="2b6ecac2",Fd="8db76605e873aaf2fbdf41256cb24cb4";function jd(){c("scerr").style.display="none",c("ffile").click()}function Bd(){a.scanDestList=!0,ye("scan");const t=c("scanovttl");t&&(t.textContent="Scan → Shopping List");const e=c("scan-dest-hint");e&&(e.textContent="Running low? Scan to add to your shopping list.")}function zd(){a.scanDestList=!1,ye("scan");const t=c("scanovttl");t&&(t.textContent="Scan Barcode");const e=c("scan-dest-hint");e&&(e.textContent="Scan a barcode to add to your pantry or shopping list.")}function qd(){if(!a.cp)return;const t=a.cp.notFound?"Barcode "+a.cp.barcode:a.cp.name,e=parseInt(c("aqty").value)||1,n=c("aunit").value.trim(),i=t+(e>1||n?" ("+e+(n?" "+n:"")+")":""),s={id:Date.now().toString(),name:i,qty:1,checked:!1,src:"scan"};a.cp.image&&(s.image=a.cp.image),ae(s),f("Added to list: "+t),L("result"),L("scan"),a.scanDestList=!1,window.showScreen("shopping")}function Vd(){const t=c("mentry");t.style.display=t.style.display==="none"?"block":"none"}async function Wd(t){const e=t.target.files[0];if(!e)return;t.target.value="",c("scanbody").style.display="none",c("scspin").style.display="block",c("scst").textContent="Reading image…";const n=await new Promise((i,s)=>{const r=new FileReader;r.onload=o=>i(o.target.result),r.onerror=s,r.readAsDataURL(e)});try{c("scst").textContent="Detecting barcode…";const i=await new Promise((r,o)=>Quagga.decodeSingle({src:n,numOfWorkers:0,inputStream:{size:1600},decoder:{readers:["ean_reader","ean_8_reader","upc_reader","upc_e_reader","code_128_reader","code_39_reader"],multiple:!1},locate:!0},l=>{l&&l.codeResult&&l.codeResult.code?r(l.codeResult.code):o("no")}));c("scst").textContent="Found "+i+" — looking up…";const s=await qs(i);a.cp=s,c("aqty").value=1,c("aexp").value="",Un("fridge",c("rl-fridge")),Vs(s),c("scanbody").style.display="block",c("scspin").style.display="none"}catch{c("scanbody").style.display="block",c("scspin").style.display="none";const i=c("scerr");i.textContent="⚠️ No barcode detected. Try better lighting or enter manually.",i.style.display="block"}}async function Gd(){const t=c("meinp").value.trim();if(!t)return;c("scanbody").style.display="none",c("scspin").style.display="block",c("scst").textContent="Looking up…";const e=await qs(t);a.cp=e,c("aqty").value=1,c("aexp").value="",Un("fridge",c("rl-fridge")),c("meinp").value="",Vs(e),c("scanbody").style.display="block",c("scspin").style.display="none"}async function qs(t){return await Kd(t)||await Jd(t)||await Yd(t)||{barcode:t,name:"",brand:"",quantity:"",category:"General",image:null,source:null,notFound:!0}}async function Kd(t){try{const e=await fetch(`https://api.edamam.com/api/food-database/v2/parser?upc=${t}&app_id=${Hd}&app_key=${Fd}`);if(!e.ok)return null;const n=await e.json(),i=n.hints&&n.hints[0]&&n.hints[0].food||n.parsed&&n.parsed[0]&&n.parsed[0].food;if(!i)return null;const s=i.nutrients||{};return{barcode:t,name:i.label||"",brand:i.brand||"",quantity:i.servingSize?`${i.servingSize}${i.servingSizeUnit||"g"}`:"",category:i.category||"General",image:i.image||null,source:"Edamam",notFound:!1,nutrition:{calories:s.ENERC_KCAL?Math.round(s.ENERC_KCAL):null,protein:s.PROCNT?`${s.PROCNT.toFixed(1)}g`:null,fat:s.FAT?`${s.FAT.toFixed(1)}g`:null,carbs:s.CHOCDF?`${s.CHOCDF.toFixed(1)}g`:null}}}catch{}return null}async function Jd(t){try{const n=await(await fetch("https://world.openfoodfacts.org/api/v0/product/"+t+".json")).json();if(n.status===1&&n.product){const i=n.product,s=i.product_name||i.product_name_en||"";return s?{barcode:t,name:s,brand:i.brands||"",quantity:i.quantity||"",category:((i.categories_tags||[])[0]||"").replace("en:","")||"General",image:i.image_small_url||null,source:"Open Food Facts",notFound:!1,nutrition:null}:null}}catch{}return null}async function Yd(t){try{const n=await(await fetch("https://api.upcitemdb.com/prod/trial/lookup?upc="+t)).json();if(n.code==="OK"&&n.items&&n.items.length>0){const i=n.items[0];return{barcode:t,name:i.title||"",brand:i.brand||"",quantity:i.size||"",category:i.category||"General",image:(i.images||[])[0]||null,source:"UPC Item DB",notFound:!1,nutrition:null}}}catch{}return null}function Vs(t){L("scan"),c("resttl").textContent=t.notFound?"Not Found":"Product Found ✓",c("aunit").value=t.quantity||"unit";let e="";if(t.notFound)e=`<div class="nfb">⚠️ Barcode <code>${t.barcode}</code> not found. Enter name:<input class="fi" id="mnm" placeholder="Product name (required)" oninput="valAdd()" style="margin-top:10px"/></div>`,setTimeout(()=>c("addbtn").disabled=!0,0);else{const n=t.image?`<img src="${t.image}" class="pimg" onerror="this.style.display='none'"/>`:'<div class="pimg" style="display:flex;align-items:center;justify-content:center;font-size:1.8rem">🛒</div>';let i="";t.nutrition&&(t.nutrition.calories||t.nutrition.protein)&&(i=`<div class="ngrd">${[["Cal",t.nutrition.calories],["Protein",t.nutrition.protein],["Fat",t.nutrition.fat],["Carbs",t.nutrition.carbs]].map(([s,r])=>`<div class="nb"><div class="nv">${r||"—"}</div><div class="nl">${s}</div></div>`).join("")}</div>`),e=`<div class="pcard"><div class="phdr">${n}<div style="flex:1"><div class="pnm">${t.name}</div>${t.brand?`<div class="pbr">${t.brand}</div>`:""}<div class="pbc">${t.barcode}</div><span class="bdg">${t.category}</span>${t.source?`<span class="srcb">${t.source}</span>`:""}</div></div>${i}</div>`,setTimeout(()=>c("addbtn").disabled=!1,0)}c("resbody").innerHTML=e,ye("result")}function Un(t,e){a.selR=t,document.querySelectorAll("#ov-result .lbtn").forEach(n=>n.classList.remove("sel")),e&&e.classList.add("sel")}function Qd(){const t=c("mnm");c("addbtn").disabled=!(t&&t.value.trim())}async function Xd(){if(!a.cp)return;const t=c("mnm"),e=a.cp.notFound?t&&t.value.trim()||"":a.cp.name;if(!e)return;const n=c("aunit").value.trim()||"unit",i=Math.max(1,parseInt(c("aqty").value)||1),s=c("aexp").value||null,r="item-"+a.cp.barcode.replace(/\W/g,"-"),o=a.inv.find(l=>l.id===r);await oe({id:r,barcode:a.cp.barcode,name:e,brand:a.cp.brand||"",unit:n,qty:o?o.qty+i:i,location:a.selR,category:a.cp.category||"General",image:a.cp.image||null,source:a.cp.source||null,nutrition:a.cp.nutrition||null,expiry:s,addedAt:o?o.addedAt:new Date().toLocaleDateString()}),f(o?`+${i} added to ${e}`:`${e} added!`),a.cp=null,L("result")}function Zd(t){const e=c("aqty");e.value=Math.max(1,(parseInt(e.value)||1)+t)}let Ce=null,Ii=0,H=null;function eu(){document.addEventListener("touchstart",t=>{const e=t.target.closest(".swipe-inner");!e||!e.closest(".swipe-wrap")||a.selectMode||(Ce=e,Ii=t.touches[0].clientX,e.classList.add("swiping"))},{passive:!0}),document.addEventListener("touchmove",t=>{if(!Ce)return;const e=t.touches[0].clientX-Ii,n=Math.max(-80,Math.min(0,e));Ce.style.transform=`translateX(${n}px)`,Math.abs(e)>8&&t.preventDefault()},{passive:!1}),document.addEventListener("touchend",()=>{if(!Ce)return;const t=Ce,e=t.closest(".swipe-wrap");t.classList.remove("swiping"),(parseFloat(t.style.transform.replace("translateX(",""))||0)<-50?(t.style.transform="translateX(-80px)",e==null||e.classList.add("open"),H&&H!==e&&pn(H),H=e):(t.style.transform="translateX(0)",e==null||e.classList.remove("open"),H===e&&(H=null)),Ce=null}),document.addEventListener("touchstart",t=>{if(!H||t.target.closest(".swipe-del"))return;const e=t.target.closest(".swipe-inner");e&&e.closest(".swipe-wrap")===H||(pn(H),H=null)},{passive:!0})}function pn(t){const e=t==null?void 0:t.querySelector(".swipe-inner");e&&(e.style.transform="translateX(0)"),t==null||t.classList.remove("open")}async function tu(t,e){const n=c("sw-"+t);n&&(n.style.opacity="0.5"),e==="shop"?await Ot(t):(await An(t),f("Item removed"))}function nu(t,e){const n=c("sw-"+t);if(n){const i=n.querySelector(".swipe-inner");if((parseFloat((i.style.transform||"").replace("translateX(",""))||0)<-10){pn(n),H=null;return}}if(a.selectMode){a.selectedIds.has(t)?(a.selectedIds.delete(t),n==null||n.classList.remove("selected")):(a.selectedIds.add(t),n==null||n.classList.add("selected")),Ht();return}e==="shop"?window.togShop(t):window.openAdj(t)}function iu(){if(a.selectMode==="shop"){De();return}a.selectMode&&De(),a.selectMode="shop",a.selectedIds.clear(),document.querySelectorAll("#shlist .swipe-wrap").forEach(e=>e.classList.add("selecting"));const t=c("sh-selbtn");t&&(t.classList.add("active"),t.textContent="Cancel"),Ht()}function su(){if(a.selectMode==="inv"){De();return}a.selectMode&&De(),a.selectMode="inv",a.selectedIds.clear(),document.querySelectorAll("#ibody .swipe-wrap").forEach(e=>e.classList.add("selecting"));const t=c("inv-selbtn");t&&(t.classList.add("active"),t.textContent="Cancel"),Ht()}function De(){a.selectMode=null,a.selectedIds.clear(),document.querySelectorAll(".swipe-wrap.selecting").forEach(n=>n.classList.remove("selecting","selected"));const t=c("sh-selbtn");t&&(t.classList.remove("active"),t.textContent="Select");const e=c("inv-selbtn");e&&(e.classList.remove("active"),e.textContent="Select"),Ht()}async function ru(){if(!a.selectedIds.size)return;const t=[...a.selectedIds],e=a.selectMode;De(),e==="shop"?await Promise.all(t.map(n=>Ot(n))):await Promise.all(t.map(n=>An(n))),f(`Removed ${t.length} item${t.length!==1?"s":""} 🗑`)}function Ht(){const t=c("multi-bar");if(!t)return;const e=a.selectedIds.size,n=c("multi-count");n&&(n.textContent=e),a.selectMode?t.classList.add("visible"):t.classList.remove("visible")}const ou=["Chicken","Beef","Fish","Vegetarian","Vegan","Quick","Kids","Healthy","Batch Cook","Date Night"];function Ws(t){return"chip-"+t.split(" ").join("-")}function Gs(){const t=c("recChips");t&&(t.innerHTML=ou.map(e=>`<button onclick="toggleChip('${e}')" id="${Ws(e)}" style="padding:5px 10px;border-radius:20px;border:1px solid var(--bd);background:var(--s2);color:var(--tx2);font-size:.78rem;cursor:pointer;transition:all .15s">${e}</button>`).join(""))}function au(t){const e=c(Ws(t));window._activeChips.has(t)?(window._activeChips.delete(t),e&&(e.style.background="var(--s2)",e.style.color="var(--tx2)",e.style.borderColor="var(--bd)")):(window._activeChips.add(t),e&&(e.style.background="var(--ac)",e.style.color="#000",e.style.borderColor="var(--ac)")),Ks()}function Ks(){const t=c("recPicker"),e=c("recFilter")?c("recFilter").value.trim().toLowerCase():"",n=[...window._activeChips].map(r=>r.toLowerCase()),s=[...a.recs].sort((r,o)=>(o.cookCount||0)-(r.cookCount||0)).filter(r=>{const o=(r.name+" "+(r.description||"")+" "+(r.tags||[]).join(" ")).toLowerCase(),l=e?e.split(/\s+/).every(u=>o.includes(u)):!0,d=n.every(u=>o.includes(u));return l&&d});t.innerHTML='<option value="">— pick a saved recipe —</option>'+s.map(r=>`<option value="${r.id}">${r.name}</option>`).join(""),window._pickedRec=null,c("mealMinp").value=""}function cu(t,e){a.md=t,c("mealMttl").textContent="Meal for "+e,c("mealMinp").value=a.mp[t]||"",window._pickedRec=null,window._activeChips=new Set;const n=c("recFilter");n&&(n.value=""),Gs();const i=c("recPicker");if(a.recs&&a.recs.length){const s=[...a.recs].sort((l,d)=>(d.cookCount||0)-(l.cookCount||0));i.innerHTML='<option value="">— pick a saved recipe —</option>'+s.map(l=>`<option value="${l.id}">${l.name}</option>`).join("");const r=a.mp[t]||"",o=s.find(l=>l.name===r);i.value=o?o.id:"",c("recPickerWrap").style.display="block"}else c("recPickerWrap").style.display="none";c("mealM").classList.add("active"),setTimeout(()=>c("mealMinp").focus(),100)}function lu(t){if(!t){window._pickedRec=null,c("mealMinp").value="";return}const e=a.recs.find(n=>n.id===t);e&&(window._pickedRec=e,c("mealMinp").value=e.name)}function Hn(){c("mealM").classList.remove("active")}async function du(){const t=c("mealMinp").value.trim();if(await Ie(a.md,t||null),window._pickedRec&&window._pickedRec.description){const e=window._pickedRec.description,n=a.inv.map(o=>o.name.toLowerCase()),i=a.shop.map(o=>o.name.toLowerCase()),s=e.split(/[\n,]/).map(o=>o.replace(/^[\d\/\s]*(cup|tbsp|tsp|oz|lb|g|kg|ml|l|clove|slice|piece|bunch|head|can|package|pkg)s?\.?\s*/gi,"").replace(/^[\d]+\s*/,"").trim()).filter(o=>o.length>1&&o.length<60);let r=0;for(const o of s){if(/\b(add|mix|heat|cook|bake|stir|boil|fry|slice|chop|dice|combine|place|pour|season|serve|preheat|bring|remove|let|set|transfer)\b/i.test(o))continue;const l=o.replace(/^[-•*]\s*/,"").trim();if(!l||l.length<2)continue;const d=l.toLowerCase();n.some(u=>u.includes(d)||d.includes(u))||i.some(u=>u===d)||(await ae({id:Date.now().toString()+Math.random().toString(36).slice(2),name:l,qty:1,checked:!1,src:"recipe"}),r++)}r>0&&f(`Added ${r} ingredient${r!==1?"s":""} to shopping list 🛒`)}window._pickedRec=null,Hn(),Se(),ct(),Fe()}async function uu(){await Ie(a.md,null),Hn(),Se(),ct(),Fe()}function hu(t){const e=a.mp[t];e&&(a.cn=e,a.nr=0,c("cookedNm").textContent=e,c("cnotes").value="",Je("cstars",0),c("cookedM").classList.add("active"))}async function fu(){await Rs(a.cn,he()),await Ie(he(),null),c("cookedM").classList.remove("active"),Se(),Fe(),f("Meal logged!")}async function pu(){var i;const t=c("cnotes").value.trim(),e=(i=c("tog-leftover"))==null?void 0:i.classList.contains("on");await Rs(a.cn,he());const n=a.recs.find(s=>s.name.toLowerCase()===a.cn.toLowerCase());n?await ke({...n,cookCount:(n.cookCount||0)+1,lastCooked:he()}):await ke({id:"rec-"+Date.now(),name:a.cn,rating:a.nr,favorited:!1,notes:t,description:"",source:"Meal Plan",tags:[],cookCount:1,savedAt:new Date().toLocaleDateString(),lastCooked:he()}),e&&await Ie(Cl(),a.cn+" (leftovers)"),await Ie(he(),null),c("cookedM").classList.remove("active"),Se(),Fe(),f(e?"Saved! Leftovers planned for tomorrow 🥡":"Saved to recipes! ⭐")}function mu(t){c("schedNm").textContent=t;const e=["S","M","T","W","T","F","S"],n=new Date;n.setHours(0,0,0,0),c("schedWk").innerHTML=He().map((i,s)=>{const r=i.toISOString().split("T")[0],o=i.getTime()===n.getTime(),l=a.mp[r];return`<div class="wd${o?" today":""}${l?" hm":""}" onclick="schedSet('${r}','${t}')"><div class="wdn">${e[s]}</div><div class="wdd">${i.getDate()}</div>${l?`<div class="wdm">${l.substring(0,8)}…</div>`:""}</div>`}).join(""),c("schedM").classList.add("active")}async function gu(t,e){await Ie(t,e),c("schedM").classList.remove("active"),Se(),Fe(),f("Scheduled! 📅")}function yu(){const t=s=>c(s),e=(s,r)=>{const o=t(s);o&&(o.value=r||"")};e("setName",a.cfg.name),e("setAdults",a.cfg.adults),e("setKids",a.cfg.kids),e("setOther",a.cfg.other),e("setCuisines",a.cfg.cuisines),e("setCookTime",a.cfg.cookTime),e("setZipcode",a.cfg.zipcode);const n=(s,r)=>{const o=t(s);o&&o.classList.toggle("on",!!r)};n("tg-nopork",a.cfg.nopork),n("tg-noshellfish",a.cfg.noshellfish),n("tg-vegetarian",a.cfg.vegetarian),n("tg-glutenfree",a.cfg.glutenfree),n("tg-notif",a.cfg.notif);const i=c("notifTimeRow");i&&(i.style.display=a.cfg.notif?"block":"none"),e("setNotifTime",a.cfg.notifTime||"8"),e("setNotifDays",String(a.cfg.notifDays||3)),jn(),Ys()}async function vu(){a.cfg={...a.cfg,name:c("setName").value.trim(),adults:c("setAdults").value.trim(),kids:c("setKids").value.trim(),nopork:c("tg-nopork").classList.contains("on"),noshellfish:c("tg-noshellfish").classList.contains("on"),vegetarian:c("tg-vegetarian").classList.contains("on"),glutenfree:c("tg-glutenfree").classList.contains("on"),other:c("setOther").value.trim(),cuisines:c("setCuisines").value.trim(),cookTime:c("setCookTime").value,zipcode:c("setZipcode")?c("setZipcode").value.trim():"",notif:c("tg-notif").classList.contains("on"),notifTime:c("setNotifTime")?c("setNotifTime").value:"8",notifDays:parseInt(c("setNotifDays")?c("setNotifDays").value:"3")},await xn(),a.cfg.notif&&Js(),f("Settings saved!"),L("settings"),$n()}async function wu(){var e,n;const t=((n=(e=c("setZipcode"))==null?void 0:e.value)==null?void 0:n.trim())||"";a.cfg={...a.cfg,zipcode:t},await xn(),f("Saved!")}async function bu(t){if(!t.classList.contains("on")){if(!("Notification"in window)){f("Notifications not supported on this browser");return}if(Notification.permission==="denied"){f("Notifications blocked — enable in browser settings");return}if(Notification.permission!=="granted"&&await Notification.requestPermission()!=="granted"){f("Notifications permission denied");return}}t.classList.toggle("on");const n=c("notifTimeRow");n&&(n.style.display=t.classList.contains("on")?"block":"none")}function Iu(){if(Notification.permission!=="granted"){f("Enable notifications first");return}const t=a.inv.filter(n=>{const i=G(n.expiry);return i&&(i.c==="expiring"||i.c==="expired")});if(!t.length){new Notification("Kitchen 🧺",{body:"No items expiring soon — you're all good!"});return}const e=t.slice(0,3).map(n=>n.name).join(", ");new Notification("Kitchen 🧺 — Expiring Soon",{body:`${e}${t.length>3?" + "+(t.length-3)+" more":""} need attention`})}function Js(){if(!a.cfg.notif||Notification.permission!=="granted")return;const t=parseInt(localStorage.getItem("ks-lastnotif")||"0"),e=Date.now();if(e-t<864e5)return;localStorage.setItem("ks-lastnotif",e.toString());const n=a.cfg.notifDays||3,i=a.inv.filter(r=>{if(!G(r.expiry))return!1;const l=new Date(r.expiry+"T00:00:00"),d=new Date;return d.setHours(0,0,0,0),Math.round((l-d)/864e5)<=n});if(!i.length)return;const s=i.slice(0,3).map(r=>r.name).join(", ");new Notification("Kitchen 🧺 — Expiring Soon",{body:`${s}${i.length>3?" + "+(i.length-3)+" more":""} expiring in ${n} days or less`})}function Fn(){return x("ks-hhs")||[a.hid]}async function Ys(){const t=D();if(t)try{const e=await E(`households/${a.hid}`);if(!e)return;const n=e.ownerUid===t.uid,i=c("hhInviteCode");if(i&&(i.textContent=e.inviteCode||"—"),e.inviteCode&&n)try{await w(`household_codes/${e.inviteCode}`,{householdId:a.hid})}catch{}const s=c("regenCodeBtn");s&&(s.style.display=n?"":"none");const r=c("hhMembers");r&&e.members&&(r.innerHTML=e.members.map(o=>{const l=o.uid===t.uid,d=o.role==="owner"?"Owner":"Member",u=n&&!l?`<button onclick="event.stopPropagation();removeMemberFromHH('${o.uid}')" style="background:none;border:none;color:var(--rd);cursor:pointer;font-size:.78rem;padding:4px 8px">Remove</button>`:"";return`<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;background:var(--card);border:1px solid var(--b2);border-radius:10px;margin-bottom:6px">
          ...
          ${u}
        </div>`}).join(""))}catch(e){console.error("renderHouseholdInfo error:",e)}}async function ku(){var e;const t=(e=c("hhInviteCode"))==null?void 0:e.textContent;if(!(!t||t==="—"))try{await navigator.clipboard.writeText(t),f("Invite code copied!")}catch{f("Couldn't copy — try manually")}}async function _u(){var n;const t=(n=c("hhInviteCode"))==null?void 0:n.textContent;if(!t||t==="—")return;const e=`Join my kitchen on Kitchen app! Use invite code: ${t} at https://pantry-app-zeta-six.vercel.app`;if(navigator.share)try{await navigator.share({text:e})}catch{}else try{await navigator.clipboard.writeText(e),f("Share text copied to clipboard!")}catch{f("Couldn't share — try manually")}}async function Su(){if(confirm("Regenerate invite code? The old code will stop working."))try{const t=await hl(a.hid);if(t){const e=c("hhInviteCode");e&&(e.textContent=t),f("New invite code generated!")}}catch(t){console.error("regenInviteCode error:",t),f("Failed to regenerate code")}}async function Eu(t){if(confirm("Remove this member from the household?"))try{await fl(a.hid,t),f("Member removed"),Ys()}catch(e){console.error("removeMemberFromHH error:",e),f("Failed to remove member")}}async function Cu(){var i,s,r;const t=(r=(s=(i=c("newHHCode"))==null?void 0:i.value)==null?void 0:s.trim())==null?void 0:r.toUpperCase();if(!t)return;const e=D();if(!e){f("Sign in first");return}const n=c("newHHCode");n.disabled=!0;try{const o=await Ls(t,e);if(!o){f("Invalid invite code. Check and try again."),n.disabled=!1;return}const l=Fn();l.includes(o)||l.push(o),z("ks-hhs",l),c("newHHCode").value="",jn(),f("Household joined!")}catch(o){console.error("addHousehold error:",o),f("Failed to join household")}n.disabled=!1}function Tu(t){t!==a.hid&&(localStorage.setItem("ks-h",t),location.reload())}async function xu(t){if(t===a.hid){f("Can't remove active household");return}const e=D();if(e)try{const i=await E(`users/${e.uid}`);if(i){const r=(i.householdIds||[]).filter(o=>o!==t);await w(`users/${e.uid}`,{...i,householdIds:r,id:void 0})}const s=await E(`households/${t}`);if(s){const r=(s.members||[]).filter(l=>l.uid!==e.uid),o=(s.memberUids||[]).filter(l=>l!==e.uid);await w(`households/${t}`,{...s,members:r,memberUids:o,id:void 0})}}catch(i){console.error("removeHousehold error:",i)}const n=Fn().filter(i=>i!==t);z("ks-hhs",n),jn()}async function jn(){const t=Fn().filter(i=>i!==a.hid),e=c("hhList");if(!e)return;if(!t.length){e.innerHTML='<div style="font-size:.82rem;color:var(--mt);padding:10px 0">No other households yet.</div>';return}const n=[];for(const i of t){let s=i;try{const r=await E(`households/${i}`);r!=null&&r.name&&(s=r.name)}catch{}n.push({id:i,name:s})}e.innerHTML=n.map(({id:i,name:s})=>`<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;background:var(--card);border:1px solid var(--b2);border-radius:10px;margin-bottom:6px;cursor:pointer" onclick="switchHousehold('${i}')">
      <div>
