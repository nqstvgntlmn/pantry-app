...
`),e=p("expbox");e&&(e.textContent=n||"No items yet.")}let Ue=null,po=!1,ys="";function Gb(){if(!(window.SpeechRecognition||window.webkitSpeechRecognition))return;const e=p("shopAddMicOpt");e&&(e.style.display="")}function Cu(n){const e=p("micstatus");e&&e.classList.toggle("visible",n)}function tf(){if(po&&Ue){Ue.stop();return}const n=window.SpeechRecognition||window.webkitSpeechRecognition;if(!n){D("Voice input not supported");return}Ue=new n,Ue.lang="en-US",Ue.interimResults=!0,Ue.maxAlternatives=1,Ue.continuous=!1,ys="",po=!0,Cu(!0),Ue.onresult=e=>{let t="";for(let i=e.resultIndex;i<e.results.length;i++){const r=e.results[i][0].transcript;e.results[i].isFinal?ys+=r:t+=r}const s=p("shi");s&&(s.value=(ys+t).trim())},Ue.onerror=e=>{e.error!=="no-speech"&&e.error!=="aborted"&&D("Couldn't hear that — try again")},Ue.onend=()=>{const e=(ys||"").trim();if(po=!1,Ue=null,ys="",Cu(!1),e){let t=e,s=1;const i=e.match(/^(\d+)\s+(.+)/),r=e.match(/^(.+?)\s*[x×]\s*(\d+)$/i);r?(t=r[1].trim(),s=parseInt(r[2],10)||1):i&&(t=i[2].trim(),s=parseInt(i[1],10)||1);const a={id:Date.now().toString(),name:t,qty:s,checked:!1,src:"manual"};He(a),D(`Added "${e}" 🎤`),Fa(a.id,t,"shop")}},Ue.start()}function bi(n){const e=n.qty||1,t=`<span class="sh-qty${e===1?" sh-qty-one":""}" onclick="event.stopPropagation();openShQty('${n.id}')"> × ${e}</span>`,s=n.image?`<img src="${n.image}" class="sh-thumb" alt="" onerror="this.style.display='none'"/>`:"";return`<div class="swipe-wrap" id="sw-${n.id}" data-id="${n.id}" data-list="shop">
    ...
    <div class="swipe-del" onclick="swipeDelItem('${n.id}','shop')"><span>🗑</span>Delete</div>
      ...
      Just add "${e}" as typed
    </button>`,s.innerHTML=g,window._enrichCtx={itemId:n,query:e,list:t,results:f}}catch(c){console.warn("Text search failed:",c),Zi()}}function Zi(){const n=p("enrichBackdrop"),e=p("enrichSheet");n&&n.classList.remove("active"),e&&e.classList.remove("active"),window._enrichCtx=null}function Zb(n){const e=window._enrichCtx;if(!e)return;const t=e.results[n];if(t){if(e.list==="shop"){const s=d.shop.find(i=>i.id===e.itemId);s&&He({...s,name:t.name,brand:t.brand||"",image:t.image||null,category:t.category||"",nutrition:t.nutrition||null,source:t.source||"search"})}else if(e.list==="inv"){const s=d.inv.find(i=>i.id===e.itemId);s&&qe({...s,name:t.name,brand:t.brand||"",image:t.image||null,category:t.category||s.category,nutrition:t.nutrition||null,source:t.source||"search"})}Zi(),D(`Updated with "${t.name}" ✓`)}}function nf(n){if(!d.hid||!n)return;const e=Date.now().toString(36)+Math.random().toString(36).slice(2,6);X(`households/${d.hid}/completed_items/${e}`,{name:n,completedAt:new Date().toISOString()}).catch(t=>console.warn("recordCompleted error:",t))}function eT(n){const e=d.shop.find(s=>s.id===n);if(!e)return;const t=!e.checked;He({...e,checked:t}),t&&nf(e.name)}function tT(n,e){n.stopPropagation();const t=p("sne-"+e),s=p("sni-"+e);if(!t)return;t.classList.toggle("open")&&s&&(s.focus(),s.setSelectionRange(s.value.length,s.value.length))}function nT(n){const e=p("sni-"+n);if(!e)return;const t=d.shop.find(i=>i.id===n);if(!t)return;const s=e.value.trim();s!==(t.note||"")&&He({...t,note:s})}function sT(n){const e=p("sqe-"+n),t=p("sqi-"+n);if(!e)return;e.classList.toggle("open")&&t&&(t.focus(),t.select())}function iT(n,e){const t=p("sqi-"+n);if(!t)return;const s=Math.max(1,(parseInt(t.value,10)||1)+e);t.value=s,sf(n)}function sf(n){const e=p("sqi-"+n);if(!e)return;const t=d.shop.find(i=>i.id===n);if(!t)return;const s=Math.max(1,parseInt(e.value,10)||1);s!==(t.qty||1)&&He({...t,qty:s})}function rT(){d.aisleMode=!d.aisleMode;const n=p("aislebtn");n&&(n.style.background=d.aisleMode?"var(--ac)":"",n.style.color=d.aisleMode?"var(--bg)":""),Zs()}function oT(n){["list","deals"].forEach(s=>{const i=p("shtab-"+s);i&&i.classList.remove("active");const r=p("sh-"+s+"-body");r&&(r.style.display="none")});const e=p("shtab-"+n);e&&e.classList.add("active");const t=p("sh-"+n+"-body");t&&(t.style.display="block"),n==="deals"&&rf()}function aT(){const n=d.shop.filter(s=>!s.checked);if(!n.length){D("List is empty!");return}const t=`🛒 Shopping List

`+n.map(s=>{let i="• "+s.name;return(s.qty||1)>1&&(i+=" × "+s.qty),s.price&&(i+=" (~$"+s.price+")"),i}).join(`
`);navigator.share?navigator.share({title:"Shopping List",text:t}).catch(()=>{}):navigator.clipboard&&navigator.clipboard.writeText(t).then(()=>D("List copied!"))}function cT(){const n=d.shop.filter(t=>t.checked);if(!n.length){D("No completed items!");return}const e=p("atk-body");e.innerHTML=`<div style="padding:16px">
  ...
  </div>`,vt("atk")}function lT(n,e,t){const s=p("atk-"+n);s.dataset.loc=e,s.querySelectorAll(".atk-loc button").forEach(i=>i.classList.remove("sel")),t.classList.add("sel")}async function uT(){const n=d.shop.filter(s=>s.checked),e=new Date().toLocaleDateString();let t=0;for(const s of n){const i=p("atk-"+s.id);if(!i)continue;const r=i.dataset.loc||Bh(s.name),a=d.inv.find(u=>u.name.toLowerCase()===s.name.toLowerCase()),c=s.qty||1;await qe({id:a?a.id:"inv-"+Date.now()+"-"+Math.random().toString(36).slice(2),name:a?a.name:s.name,qty:a?a.qty+c:c,unit:a?a.unit:"unit",location:r,category:a?a.category:Os({name:s.name}),addedAt:a?a.addedAt:e,brand:a?a.brand:s.brand||"",expiry:a?a.expiry:null,image:a?a.image:s.image||null,source:"shopping"}),await hr(s.id),t++}Ae("atk"),D(`${t} item${t!==1?"s":""} added to your kitchen! 🧺`)}async function hT(){const n=Qn().map(i=>{const r=i.toISOString().split("T")[0];return d.mp[r]?`${i.toLocaleDateString("en-US",{weekday:"short"})}: ${d.mp[r]}`:""}).filter(Boolean).join(", ");if(!n){D("No meals planned yet!");return}const e=d.inv.map(i=>`${i.name} (${i.qty} ${i.unit})`).join(", "),t=document.querySelector('[onclick="buildList()"]'),s=t?t.textContent:"";t&&(t.disabled=!0,t.textContent="⏳ Thinking…");try{const r=await(await fetch("/api/proxy",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:500,messages:[{role:"user",content:`Week meals: ${n}
Already have: ${e}
...
Recipe: ${e.description}`}]})})).json(),r=(i.content&&i.content[0]&&i.content[0].text||"").replace(/```json|```/g,"").trim(),c=JSON.parse(r).filter(u=>!t.some(f=>f.includes(u.toLowerCase())||u.toLowerCase().includes(f)));if(!c.length){D("All ingredients already in pantry ✓");return}for(const u of c)await He({id:"shop-"+Date.now()+"-"+Math.random().toString(36).slice(2),name:u,qty:1,checked:!1,src:"recipe"});D(`Added ${c.length} ingredient${c.length!==1?"s":""} to shopping list 🛒`),Ae("erec"),window.showScreen("shopping")}catch{D("Couldn't parse ingredients")}}function zT(n,e){d.nr=n,e==="r"?Cs("rstars",n):e==="c"?Cs("cstars",n):e==="e"&&Cs("estars",n)}async function GT(n){const e=d.recs.find(r=>r.id===n);if(!e)return;const t=!e.isPublic,s=xe(),i=(s==null?void 0:s.displayName)||localStorage.getItem("ks-who")||"Anonymous";t?(await gv(e,i,d.hid),D("Recipe shared with the community!")):(await yv(e.id),D("Recipe removed from community")),await Ft({...e,isPublic:t})}async function qa(){const n=p("rbody");if(n){n.innerHTML='<div class="es"><div class="ei">🌍</div><p>Loading community recipes…</p></div>';try{d.comRecs=await vv(),Ha()}catch(e){console.error("loadCommunity:",e),n.innerHTML=`<div class="es"><div class="ei">⚠️</div><p>Couldn't load community recipes.</p></div>`}}}function KT(n){d.comCuisine=n,Ha()}function WT(n){d.comSearch=n,Ha()}function Ha(){const n=p("rbody");if(!n)return;let e=[...d.comRecs];if(d.comCuisine&&d.comCuisine!=="all"&&(e=e.filter(i=>(i.cuisine||"").toLowerCase().includes(d.comCuisine.toLowerCase())||(i.tags||[]).some(r=>r.toLowerCase().includes(d.comCuisine.toLowerCase())))),d.comSearch){const i=d.comSearch.toLowerCase();e=e.filter(r=>(r.title||"").toLowerCase().includes(i)||(r.tags||[]).join(" ").toLowerCase().includes(i)||(r.cuisine||"").toLowerCase().includes(i)||(r.authorName||"").toLowerCase().includes(i))}e.sort((i,r)=>new Date(r.createdAt||0)-new Date(i.createdAt||0));const t=p("rsub");t&&(t.textContent=e.length+" community recipe"+(e.length!==1?"s":""));let s=`<div style="margin-bottom:14px">
    ...
    </div>`,vt("erec")}async function JT(n){if(!xe()){D("Sign in to like recipes");return}const t=d.myLikes.has(n);try{await _v(n,t),t?d.myLikes.delete(n):d.myLikes.add(n);const s=d.comRecs.find(r=>r.id===n);s&&(s.likes=(s.likes||0)+(t?-1:1));const i=p("com-like-btn");if(i){const r=d.myLikes.has(n);i.className=`btn ${r?"bp":"bs"} bsm`,i.innerHTML=`${r?"❤️":"🤍"} ${(s==null?void 0:s.likes)||0} Like${((s==null?void 0:s.likes)||0)!==1?"s":""}`}D(t?"Like removed":"Liked!")}catch(s){console.error("likeComRecipe:",s),D("Couldn't update like")}}async function YT(n){if(!xe()){D("Sign in to save recipes");return}const t=d.comRecs.find(s=>s.id===n);if(t)try{await Ev(t),D("Recipe saved to your kitchen! 📖"),Ae("erec")}catch(s){console.error("saveComToKitchen:",s),D("Couldn't save recipe")}}async function XT(n){var r;const e=xe();if(!e){D("Sign in to comment");return}const t=p("com-cmt-input"),s=(r=t==null?void 0:t.value)==null?void 0:r.trim();if(!s)return;const i=e.displayName||localStorage.getItem("ks-who")||"Anonymous";try{const a=await wv(n,s,i);t.value="";const c=p("com-comments");c&&a&&(c.querySelector("div[style*='color:var(--mt)']")&&!c.querySelector("div[style*='border-bottom']")&&(c.innerHTML=""),c.innerHTML+=`<div style="padding:10px 0;border-bottom:1px solid var(--b1)">
        <div style="display:flex;justify-content:space-between;align-items:center">
...
:::END:::
Always use this format so the app can offer a one-tap save button. You can include normal text before/after recipe blocks.`}function nE(n){return n.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>").replace(/^\d+\.\s+(.+)$/gm,"<li>$1</li>").replace(/^[-•]\s+(.+)$/gm,"<li>$1</li>").replace(/\n/g,"<br>")}async function cf(){const n=p("chi"),e=n.value.trim();if(!e)return;n.value="",lf(n),d.chat.push({role:"user",content:e}),mo("user",e);const t=p("csb");t&&(t.disabled=!0);const s="thinking-"+Date.now(),i=p("chmsgs");i.innerHTML+=`<div class="cb asst thinking" id="${s}">Thinking…</div>`,i.scrollTop=i.scrollHeight;try{const a=await(await fetch("/api/proxy",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:800,system:tE(),messages:d.chat.map(f=>({role:f.role,content:f.content}))})})).json(),c=a.content&&a.content[0]&&a.content[0].text||"Sorry, I couldn't process that.",u=p(s);u&&u.remove(),d.chat.push({role:"assistant",content:c}),mo("assistant",c)}catch{const a=p(s);a&&a.remove(),mo("assistant","Sorry, I couldn't reach Claude. Check your connection and try again.")}t&&(t.disabled=!1)}function sE(n){const e=[];return{cleanText:n.replace(/:::RECIPE:::\s*([\s\S]*?)\s*:::END:::/g,(s,i)=>{try{const r=JSON.parse(i.trim());r.title&&e.push(r)}catch{}return""}).trim(),recipes:e}}function iE(n){const e=JSON.stringify(n).replace(/&/g,"&amp;").replace(/"/g,"&quot;"),t=(n.ingredients||"").split(`
